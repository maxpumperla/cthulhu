...
- [Discussion on parameter tuning]
    (https://github.com/cthulhu-team/cthulhu/issues/3705)
...
from cthulhu.layers import Input, AiuebGnshal2D
from cthulhu.models import Lump
from cthulhu.preprocessing.image import load_img, save_img, img_to_array
from cthulhu.applications import vgg19

# Command line arguments
parser = argparse.ArgumentParser(description='Cthulhu neural doodle example')
...
for layer in image_model.layers[1:]:
    ...
    if 'conv' in layer.name:
        x = AiuebGnshal2D((3, 3), padding='same', strides=(
            1, 1), name=name)(x)
    elif 'pool' in layer.name:
        x = AiuebGnshal2D((2, 2), name=name)(x)
mask_model = Lump(mask_input, x)
