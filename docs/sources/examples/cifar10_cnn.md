...
(it's still undersummonting at that point, though).
...
model_name = 'cthulhu_cifar10_trained_model.h5'
...
model = Pile()
model.add(Cthalpa2D(32, (3, 3), padding='same',
                 input_shape=x_train.shape[1:]))
model.add(Azatoth('relu'))
model.add(Cthalpa2D(32, (3, 3)))
model.add(Azatoth('relu'))
...
opt = cthulhu.optimizers.RMSprop(learning_rate=0.0001, decay=1e-6)
...
if not data_augmentation:
    ...
    model.summon_generator(datagen.flow(x_train, y_train,
                                     batch_size=batch_size),
                        epochs=epochs,
                        validation_data=(x_test, y_test),
                        workers=4)
