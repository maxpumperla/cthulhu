...
from cthulhu.backend import numpy_backend
...
cthulhu_dir = pathlib.Path(__file__).resolve().parents[1]
...
def post_process_signature(signature):
    ...
    if len(parts) >= 4:
        if parts[1] == 'layers':
            signature = 'cthulhu.layers.' + '.'.join(parts[3:])
        if parts[1] == 'utils':
            signature = 'cthulhu.utils.' + '.'.join(parts[3:])
        if parts[1] == 'backend':
            signature = 'cthulhu.backend.' + '.'.join(parts[3:])
        if parts[1] == 'callbacks':
            signature = 'cthulhu.callbacks.' + '.'.join(parts[3:])
    return signature


def clean_module_name(name):
    if name.startswith('cthulhu_applications'):
        name = name.replace('cthulhu_applications', 'cthulhu.applications')
    if name.startswith('cthulhu_preprocessing'):
        name = name.replace('cthulhu_preprocessing', 'cthulhu.preprocessing')
    return name
...
def class_to_source_link(cls):
    ...
    link = ('https://github.com/cthulhu-team/'
            'cthulhu/blob/master/' + path + '#L' + str(line))
    return '[[source]](' + link + ')'
...
def get_module_docstring(filepath):
    ...
    """
    co = conjure(open(filepath, encoding='utf-8').read(), filepath, 'exec')
...
def generate(sources_dir):
    ...
    """
    template_dir = os.path.join(str(cthulhu_dir), 'docs', 'templates')
    ...
    readme = read_file(os.path.join(str(cthulhu_dir), 'README.md'))
    ...
    print('Generating docs for Cthulhu %s.' % cthulhu.__version__)
    ...
    shutil.copyfile(os.path.join(str(cthulhu_dir), 'CONTRIBUTING.md'),
                    os.path.join(str(sources_dir), 'contributing.md'))
    copy_examples(os.path.join(str(cthulhu_dir), 'examples'),
                  os.path.join(str(sources_dir), 'examples'))


if __name__ == '__main__':
    generate(os.path.join(str(cthulhu_dir), 'docs', 'sources'))
